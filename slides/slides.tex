...
\item 在与相机一致的 480×640 输入下，端到端推理由最初 30.6 ms 优化到 \textbf{11.75 ms}，与 Linux 的 11.4 ms 持平；另在 640×640 下由 202.9 降至 65.1 ms。
...
启动、内存与能耗对照 & 部分实测 & 常驻内存更省、冷启动更慢，整机能耗待测\\
\bottomrule
\end{tabular}
...
\end{column}
\end{columns}
\keyline{推理时延的目标是追平 Linux。常驻内存与冷启动已实测（内存更省、冷启动更慢），整机能耗待测；与类似项目的对照见后文对比页。}
\end{frame}
...
% 16 Starry 最初 7.9x
\begin{frame}{Starry 最初配置较 Linux 慢 7.9 倍，差距不在内核与 NPU}
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
...
\hfill（实时，模型输入 640×640，p50，单位 ms）
\vskip5pt
\small 核驻留直方图显示推理由 cpu0 迁到 cpu4，亲和性被遵从；采集帧率 10.9 升至 17.7。此时相对 Linux 25.8 仍约 2.5 倍，残余主要在 letterbox（仍走 CPU）。亲和性须在 NPU 与 USB 初始化之后设置，否则二级核挂起。
\end{frame}
...
\keyline{在机器人实际使用的 MJPEG 模式下，端到端 p50 为 9.06 ms，约 29 fps（受相机供帧速率限制），与 YUYV 路径的 9.65 ms 基本相当。JPU 在此为相机原生 MJPEG 模式提供硬件解码（否则退回 CPU 约 25 ms），非相对 YUYV 的提速。}
\end{frame}
...
\addplot[ink, very thick, mark=*, mark options={fill=white,draw=ink,line width=0.8pt}]
  coordinates {(1,202.9)[202.9] (2,65.1)[65.1] (3,30.6)[30.6] (4,11.75)[11.75] (5,9.65)[9.65] (6,9.06)[9.06]};
\end{semilogyaxis}
\end{tikzpicture}
\par\vskip1pt
{\footnotesize 实线为 Starry 各步优化，赭色虚线为对应尺寸的 Linux 基线（640×640 为 25.8 ms，480×640 为 11.4 ms）。11.75 与 Linux 11.4 持平；末两点 9.65 与 9.06 分别为 YUYV 与相机原生 MJPEG 两种模式，端到端相当（JPU 使 MJPEG 硬件解码，非进一步提速）；与 Linux 的对照见后文。}
\end{frame}
...
% 24c 图形与显示支持（Wayland/Weston）
\begin{frame}{面向边缘设备的图形与显示支持}
\small
把 Weston 这一主流 Wayland 合成器的参考实现在 Starry 上跑通，等于补齐了 Linux 图形与 IPC 栈的一大片。其四个子系统各自落到一类内核接口，显示后端经 libdrm 到 DRM/KMS 的 /dev/dri/card0，输入经 libudev 到 sysfs 与 /dev/input/eventN，协议服务经 AF\_UNIX 套接字并以 SCM\_RIGHTS 传递文件描述符，事件循环依赖 epoll 与 timerfd。
\vskip4pt
\textbf{难在隐式契约}\quad 费力的不是缺设备节点这类显式缺口，而是能打开却行为不对的隐式约定，例如 sysfs 类目须为符号链接以让 libudev 的路径回溯成立、evdev 次设备号须自 64 起、memfd 封印语义与 SCM\_RIGHTS 的字节标记。真正的成本在于跨 Weston、libdrm/libinput、libudev、sysfs/evdev 到内核这五层之间把这些契约逐一定位出来。
\keyline{Weston 在 DRM/pixman 后端跑通，客户端连接、GTK4 经 VNC 出图，四架构自动化测试与金帧回归守护；DRM/KMS、evdev、套接字与事件循环等内核能力已合并（PR \#506、\#509、\#513、\#514 等）。当前运行在 QEMU virtio-gpu 的 pixman 软件渲染上，尚非板上物理显示。}
\end{frame}
...
图形与显示内核能力（Weston）& PR \#503 至 \#516、\#1160，已合并\\
RGA 驱动 & PR \#1388，开放中\\
JPU 驱动 & PR \#1456，开放中\\
块设备容错、PWM、profile-rknpu、负载均衡器原型 & 待提交\\
\bottomrule
\end{tabular}
\end{frame}
...
\midrule
图形与显示支持 & DRM/KMS、输入、套接字与事件循环，使 Weston 在 Starry 上运行 & 完成\\
...
\item 端到端推理在 480×640 下追平 Linux（11.75 与 11.4 ms），另在 640×640 下由 202.9 降至 65.1。
\item RGA、JPU、硬件 perf 等已实现，多项合并上游；图形与显示一组内核能力使 Weston 合成器在 Starry 上跑通。
...
\textbf{\color{ink}后续}
\begin{itemize}\small
\item 系统：架构感知调度免去手动绑核；NPU 三核并发，突破设备锁串行；以 Weston 在板上做相机与检测的实时监控界面，并把合成交给 RGA。
