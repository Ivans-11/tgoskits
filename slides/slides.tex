...
% ============================================================================
%  LANGUAGE: Simplified Chinese (zh-CN). Formal technical register.
%  这份幻灯片全程使用简体中文，正式书面技术语体，不译为英文，不混入口语。
%  Build:  tectonic -X compile slides.tex   ->  slides.pdf
%  进展汇报幻灯片 · 功能挑战 · proj4 · 队伍 Posad · 清华大学
% ============================================================================
...
\end{column}
\end{columns}
\keyline{推理时延的目标是追平 Linux。启动、内存与能耗等维度尚未实测，列为后续工作，不预设结论；与类似项目的对照见第 25 页。}
\end{frame}
...
% 11 USB 串口与 reboot
\begin{frame}{USB 串口驱动与 reboot 系统调用}
\begin{itemize}
\item \textbf{USB 串口驱动}\quad 以 /dev/ttyUSB0 暴露机械臂控制器（经 CP210x USB 转串口芯片相连），程序据此读机械臂状态、下控制指令，完成定位与抓取。
\item \textbf{reboot 系统调用}\quad 使 Linux 与 Starry 的切换由插拔变为一条命令，提升迭代效率。
\end{itemize}
\vskip4pt
...
\keyline{9.65 相对 11.75 的下降来自 RGA 接管前处理。该步对应的 Linux 同配置基线尚未测得，故 9.65 不与 Linux 比较，持平结论只用 11.75 与 11.4。}
\end{frame}
...
\begin{semilogyaxis}[
  ...
  xticklabels={{YOLOv8 640×640},{绑大核},{降 480×640},{免反量化},{RGA 零拷贝}},
...
\addplot[ochre, dashed, thick, forget plot] coordinates {(2.6,11.4) (4,11.4)};
\addplot[ink, very thick, mark=*, mark options={fill=white,draw=ink,line width=0.8pt}]
  coordinates {(1,162.7)[162.7] (2,55.2)[55.2] (3,30.6)[30.6] (4,11.75)[11.75] (5,9.65)[9.65]};
\end{semilogyaxis}
\end{tikzpicture}
\par\vskip1pt
{\footnotesize 实线为 Starry 各步优化，赭色虚线为对应尺寸的 Linux 基线（640×640 为 25.8 ms，480×640 为 11.4 ms）。11.75 与 Linux 11.4 持平；9.65 为再加 RGA 零拷贝的 Starry 侧结果，其 Linux 同配置基线待补。}
\end{frame}
