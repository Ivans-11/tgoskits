...
/// Borrows the live device tree for the lifetime of the program.
///
/// The FDT is parsed once and stored in a static [`OnceLock`], so callers that
/// need to resolve several phandles can borrow it without cloning the tree.
