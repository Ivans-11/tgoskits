...
\end{axis}
\end{tikzpicture}
\caption{480$\times$640 输入下各步优化的端到端时延，自默认的 30.6\,ms 先降至与 Linux 11.4\,ms（赭色虚线）持平的 11.75\,ms，再由 RGA 与 JPU 的硬件零拷贝压到约 9\,ms（受相机供帧速率限制）。横轴只覆盖该输入尺寸，640$\times$640 输入的收敛见表~\ref{tab:affinity}}\label{fig:ladder}
\end{figure}
