...
\keyline{9.65 相对 11.75 的下降来自 RGA 接管前处理；持平结论以同尺寸的 11.75 与 11.4 为准。接入 JPU 后完整零拷贝流水线与 Linux 的对照见后文。}
\end{frame}
...
\vskip5pt
接入机器人程序后，MJPEG 由 JPU 在硬件上解码，再经 RGA 转色彩并写入 NPU 输入缓冲，三者共用同一段 dma-heap 缓冲，构成 MJPEG$\to$JPU$\to$RGA$\to$NPU 的零拷贝流水线，单帧解码 p50 约 1 至 2 ms。
\vskip5pt
...
\end{center}
\vskip3pt
中位差异来自调频而非硅片：Starry 不降频，Linux 默认 ondemand 在持续负载下把 NPU 降频（固定图连测 Linux 4.7$\to$10.6、Starry 稳定 4.8）。同一时钟下 NPU 本就相当，与 480×640 下 11.75 与 11.4 的持平印证，切到 performance 即追平。
